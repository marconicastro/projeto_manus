...
export default function Home() {
  ...
  const [exitPopupTimeLeft, setExitPopupTimeLeft] = useState(600) // 10 minutos em segundos
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = (e: MouseEvent) => {
      ...
      if (e.clientY <= 0 || e.relatedTarget === null) {
        ...
        // Prevenir que o popup apareça novamente muito rápido
        sessionStorage.setItem('exitPopupShown', 'true')
      }
    }

    // Detectar quando o usuário tenta fechar a aba/navegador
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      // Mostrar popup se ainda não foi mostrado nesta sessão
    ...
    // Detectar quando o usuário perde o foco da janela
    const handleBlur = () => {
      if (!sessionStorage.getItem('exitPopupShown')) {
        setTimeout(() => {
          if (document.hidden) {
    ...
    // Contador regressivo do popup de saída
    const exitPopupTimer = setInterval(() => {
      if (showExitPopup && exitPopupTimeLeft > 0) {
        setExitPopupTimeLeft(prev => prev - 1)
      }
    }, 1000)
  ...
  // Formatar tempo do popup
  const formatExitPopupTime = (seconds: number) => {
  ...
  return (
    <>
      {/* Exit Intent Popup - Melhorado */}
      {showExitPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 transform animate-bounce shadow-2xl border-4 border-red-500">
            <div className="text-center">
              ...
              <p className="text-gray-700 mb-6 font-semibold">
                Ficamos tristes em ver você ir... Que tal uma oferta especial apenas por ter chegado até aqui?
              </p>
              
              <div className="bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl p-6 mb-6 shadow-lg">
                <div className="text-white font-bold text-lg mb-3 animate-pulse">
                  ⚠️ OFERTA EXCLUSIVA DE SAÍDA! ÚLTIMA CHANCE!
                </div>
                <div className="text-3xl font-bold text-yellow-300 mb-3">
                  APENAS R$ 39,90
                </div>
                <div className="text-white line-through mb-2">
                  De: R$ 86,00
                </div>
                <div className="bg-yellow-400 text-red-800 px-3 py-2 rounded-full text-sm font-bold inline-block animate-pulse">
                  ⏰ OFERTA EXPIRA EM: {formatExitPopupTime(exitPopupTimeLeft)}
                </div>
                <div className="text-yellow-300 text-sm mt-2 font-semibold">
                  Economia de R$ 46,10 - Só por 10 minutos!
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Button 
                  size="lg" 
                  className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold py-4 rounded-full transform hover:scale-105 transition-all duration-200 shadow-lg"
                  onClick={handleCheckoutClick}
                >
                  🚀 QUERO MINHA OFERTA ESPECIAL! SIM!
                </Button>
                
                <button 
                  onClick={() => setShowExitPopup(false)}
                  className="text-gray-500 hover:text-gray-700 text-sm underline font-semibold"
                >
                  Não, obrigado. Realmente não quero economizar R$ 16,10...
                </button>
              </div>
            </div>
          </div>
